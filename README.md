...
目前開發版本：**P-WORLD 圖文中文攻略 Golden Test（等待手機人工驗收）**（`dev`）。
...
18. 首頁新增「快速中文攻略」入口。Golden Test 機種 `スマスロ バイオハザードRE:3` 可將 P-WORLD 官方資料區的流程、CZ、AT、Bonus 與打法圖片配進完整繁中指南；掲示板、留言、廣告、外站圖片與過小圖片不會收錄。
...
- 指南整理結構化事實、數值、自行撰寫的中文提示、來源 URL 與擷取時間；不保存完整攻略文章、留言、導覽或推薦內容。圖片保存目前僅限 `スマスロ バイオハザードRE:3` Golden Test，最多 18 張、單張上限 1 MB；不代表已取得轉載授權，也未擴張到全部 Catalog。
- Golden Test 圖片在 Vercel 有 `SUPABASE_URL` 與 server-only Supabase secret 時寫入 private `machine-guide-assets` bucket，瀏覽器只透過同源圖片 route 讀取；沒有雲端設定時暫時回退來源即時讀取。Guide 本體仍是 browser-local cache，尚未跨裝置同步。
...
- Supabase 目前保存 Machine Catalog identity records、import job audit，以及單機 Golden Test 的 private guide image assets；Session、Guide JSON cache、自訂記錄與回報仍是 browser-local，尚無跨裝置同步。
