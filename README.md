...
目前開發版本：**v0.2.6.3 – Adaptive Session UI Foundation**（`dev`，等待手機人工驗收）。
...
5. 指南保存在目前瀏覽器 localStorage，並可立即建立 Session；畫面只顯示該 Session snapshot 中 operational 的記錄能力。
...
- Session UI 已由 capability snapshot 產生；read-only／unavailable 項目只留在指南參考，不顯示空白或無作用控制項。
