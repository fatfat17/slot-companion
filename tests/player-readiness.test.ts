...
test("trip pack includes guide pages, same guide images and core player routes",()=>{
  ...
  assert.ok(guideOfflineUrls(guide).includes("/guides/10530"));assert.ok(urls.includes("/catalog"));assert.ok(urls.includes("/records"));assert.ok(urls.includes("/halls"));assert.ok(urls.includes("/help"));assert.ok(urls.includes("/api/catalog-covers/10530?source=https%3A%2F%2Fidn.p-world.co.jp%2FTEST.jpg"));assert.equal(OFFLINE_PACK_CACHE.startsWith("slot-companion-trip-pack-"),true);
});

test("service worker preserves trip packs while replacing old shell caches",()=>{const source=fs.readFileSync(new URL("../public/sw.js",import.meta.url),"utf8");assert.match(source,/TRIP_PREFIX/);assert.match(source,/!key\.startsWith\(TRIP_PREFIX\)/);assert.match(source,/"\/help"/);assert.doesNotMatch(source,/slot-companion-v0\.2\.2\.1/)});
