...
const uncertainTitleResult=(title:string):MachineIdentificationResult=>({provider:"openai",status:"uncertain",candidates:[{machineNameJa:title,machineNameZh:"",manufacturer:"不明",confidence:.84,reason:"TEST phase 2 title without catalog id",visibleEvidence:["title plate"],matchedCatalogId:null,identityBasis:"official_title_visible"}]});
...
test("L とある魔術の禁書目録2 deterministic recovery 匹配正確 Catalog",async()=>{const result=await identify("L とある魔術の禁書目録2",()=>uncertainTitleResult("L とある魔術の禁書目録2"));assert.equal(result.status,"identified");assert.equal(result.candidates[0].matchedCatalogId,"machine-th4uhu");assert.equal(result.candidates[0].machineNameJa,"スマスロ とある魔術の禁書目録2");assert.equal(result.candidates[0].manufacturer,"藤商事");assert.ok(result.debug?.shortlist.some(item=>item.id==="machine-th4uhu"))});
test("スマスロ前綴的同一正式 title 匹配同一 Catalog",async()=>{const result=await identify("スマスロ とある魔術の禁書目録2",()=>uncertainTitleResult("スマスロ とある魔術の禁書目録2"));assert.equal(result.status,"identified");assert.equal(result.candidates[0].matchedCatalogId,"machine-th4uhu")});
test("manufacturer 缺席或標為不明時不拒絕唯一完整 title",async()=>{const missingMakerEvidence={...evidence("L とある魔術の禁書目録2"),visibleManufacturerMarks:["メーカー：不明"]},provider=new FakeTwoPhaseProvider(missingMakerEvidence,()=>uncertainTitleResult("L とある魔術の禁書目録2")),result=await runIdentificationPipeline({image,provider,catalog,includeDebug:true});assert.equal(result.status,"identified");assert.equal(result.candidates[0].manufacturer,"藤商事")});
test("manufacturer 明確衝突仍降為 uncertain",async()=>{const conflictEvidence={...evidence("L とある魔術の禁書目録2"),visibleManufacturerMarks:["別メーカー"]},provider=new FakeTwoPhaseProvider(conflictEvidence,()=>uncertainTitleResult("L とある魔術の禁書目録2")),result=await runIdentificationPipeline({image,provider,catalog,includeDebug:true});assert.equal(result.status,"uncertain");assert.equal(result.candidates[0].matchedCatalogId,null);assert.match(result.debug?.phase2.decisionReasons.join(" ")??"",/manufacturer.*明確衝突/)});
test("一方通行候選不得被核心 title recovery 覆蓋",async()=>{const result=await identify("L とある魔術の禁書目録2",shortlist=>phase2Result(shortlist.find(item=>item.id==="machine-9bisnd")!));assert.equal(result.status,"uncertain");assert.notEqual(result.candidates[0].matchedCatalogId,"machine-th4uhu")});
test("續作數字 2 不可匹配無續作數字的 title",()=>assert.equal(hasVersionConflict("とある魔術の禁書目録2","とある魔術の禁書目録"),true));
test("RE、Roman numeral 與 V suffix 版本衝突保持保護",()=>{assert.equal(hasVersionConflict("BIOHAZARD RE:2","BIOHAZARD RE:3"),true);assert.equal(hasVersionConflict("TEST TITLE II","TEST TITLE III"),true);assert.equal(hasVersionConflict("TEST TITLE V","TEST TITLE V-30"),true)});
