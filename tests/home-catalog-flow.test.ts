...
test("app help is opt-in, visual and separates Slot from Pachinko",()=>{const help=fs.readFileSync(new URL("../src/app/help/page.tsx",import.meta.url),"utf8");for(const copy of ["不用全部看完","指南不會自動彈出","加入 iPhone 主畫面","SLOT 快速流程","柏青哥快速流程","拍照找機台","中文機台指南","遊玩中的紀錄","AI 陪打助手","SLOT 選台助手","結束與今日紀錄"])assert.match(help,new RegExp(copy));assert.match(help,/help-visual/);assert.match(help,/href="\/identify"/);assert.match(help,/href="\/identify\/pachinko"/);assert.match(help,/href="\/catalog\/assistant"/);assert.doesNotMatch(help,/localStorage|sessionStorage|首次.*自動/)});

test("PWA branding covers both Slot and Pachinko with a dedicated Apple icon",()=>{const manifest=fs.readFileSync(new URL("../src/app/manifest.ts",import.meta.url),"utf8"),layout=fs.readFileSync(new URL("../src/app/layout.tsx",import.meta.url),"utf8"),icon=fs.readFileSync(new URL("../src/app/icon.tsx",import.meta.url),"utf8"),appleIcon=fs.readFileSync(new URL("../src/app/apple-icon.tsx",import.meta.url),"utf8");assert.match(home,/PACHI COMPANION/);assert.match(home,/brand-mark">伴/);assert.match(manifest,/name: "打台夥伴｜日本 SLOT・柏青哥"/);assert.match(manifest,/short_name: "打台夥伴"/);assert.match(manifest,/purpose: "maskable"/);assert.match(layout,/appleWebApp: \{ capable: true, statusBarStyle: "black-translucent", title: "打台夥伴" \}/);assert.match(icon,/AppIconArtwork/);assert.match(appleIcon,/width:180,height:180/)});
