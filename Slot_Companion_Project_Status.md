...
正式 Production 基準：**release commit `16cb5a8`（打台夥伴 Brand + iPhone Icon）**
...
### 打台夥伴 Brand + iPhone Home Screen Icon（2026-09-21，Production 已發佈）
...
- 產品 commit `c5d0f7d` 已 push `dev`；release commit `16cb5a8` 已 push `main`，Vercel 顯示 **Deployment has completed**。固定正式首頁、Manifest、兩種 icon route 與 `/help#install` 均已通過自動 QA。
...
- Assistant 只接收 server 重新解析的 SLOT Catalog identity、玩家輸入、暫時照片與這台瀏覽器已有的 Machine Guide 摘要。沒有本機 Guide 時清楚顯示建立入口；沒有 ceiling／Reset 來源時，prompt 強制回覆目前沒有可靠資料，不用模型記憶補猜。
...
- 產品 commit `4aabc6c` 已 push `dev`；release commit `0d33f53` 已 push `main`，Vercel 顯示 **Deployment has completed**。固定正式 `/` 與 `/catalog` 均回 HTTP 200。
...
- 產品 commit `f2c22fc` 已 push 至 `origin/dev`；release merge commit `2927233` 已 push 至 `main`。Vercel Production 顯示 **Deployment has completed**。
- 固定正式網址 smoke：`/`、`/start`、`/pachinko`、`/pachinko/pachi-10504`、`/records`、`/halls?kind=pachinko` 全部回應 HTTP 200。遠端 migration 仍未套用，Production 目前由 repo JSON seed fallback 承接 Pachinko Catalog。
...
- 本項是自動瀏覽器 QA，不冒充使用者在日本現場的實體手機人工驗收。

### SLOT Catalog AI Selector QA（2026-09-21，自動 QA）
...
- Vercel release `0d33f53` 的 GitHub deployment status 為 success／Deployment has completed；固定 Production 首頁與 SLOT Catalog 均為 HTTP 200。
...
## Current Work
**「打台夥伴」品牌、跨平台圖示與 iPhone 加入主畫面教學已完成並發佈 Production；等待使用者在 iPhone 重新加入主畫面驗收。**
...
打台夥伴 Brand + iPhone Icon：**Completed；Production release `16cb5a8` 已部署，名稱、首頁品牌、Manifest、512px 圖示、180px Apple 圖示與安裝教學均已通過正式手機版 QA**
...
## Next Step
### SLOT AI 實戰驗證與柏青哥 AI 範圍

Status：**SLOT Session Companion 與 Catalog Selector 均完成工程版本；先由使用者驗證真實體感，再開始柏青哥 AI。**

1. 使用者在正式站以真實 Session、機台畫面與資料機履歷測試 SLOT 兩個 AI 入口，確認回答節奏、照片辨識與現場操作負擔。
2. 依實測確認是否需要保存選台分析、候選草稿或跨局記憶；目前全部保持短期、browser-local／request-only 邊界。
3. SLOT 體感確認後，再設計柏青哥 AI：以規格解釋、回轉／投入紀錄與照片問答為主，不複製 SLOT 的 Reset、天井或晚間撿台模式。
