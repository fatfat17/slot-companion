...
export default async function CatalogDetailPage({params}:{params:Promise<{id:string}>}){
  ...
  return <><PageHeader title="機種詳細" eyebrow="Machine Catalog"/><main className="page catalog-detail">
    <section className="catalog-detail-hero card"><span className="badge">{record.catalogStatus}</span><h1>{record.officialNameJa}</h1>{record.displayNameZh&&<p>{record.displayNameZh}</p>}<div className={profile?"profile-ready":"catalog-only"}>{profile?"✅ 已有既有 Machine Card":"📚 Machine Catalog 已配對，可建立機台指南"}</div></section>
