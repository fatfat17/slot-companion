...
export default async function CatalogDetailPage({params}:{params:Promise<{id:string}>}){
  ...
  return <><PageHeader title="機種詳細" eyebrow="Machine Catalog" backHref="/catalog" backLabel="返回機種資料庫"/><main className="page catalog-detail">
    ...
    <Link className="primary-button mt-3" href={`/halls?machine=${encodeURIComponent(record.officialNameJa)}`}>附近哪裡有這台</Link>
