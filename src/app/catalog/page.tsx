...
export default async function CatalogPage(){const records=await catalogRepository.list(),profiles=machines.map(machine=>({catalogId:machine.catalogId??machine.id,machineId:machine.id,nameZh:machine.nameZh})),importer=getCatalogImporterPresentation(process.env.NODE_ENV,catalogAdminEnabled()),estimatorCoverage=await getCatalogEstimatorCoverage();return<><PageHeader title="機種資料庫" eyebrow="Machine Catalog"/><CatalogLibraryClient records={records} profiles={profiles} importer={importer} estimatorEligibleIds={estimatorCoverage?.eligibleCatalogIds??null}/></>}
