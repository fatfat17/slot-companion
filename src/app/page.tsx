...
export default function Home() {
  ...
  return (
    <main className="page home-page">
      ...
      <Link href={active ? `/session/${active.id}` : "/start"} className="session-hero">
        <span className="session-icon">🎰</span>
        <span className="flex-1">
          <small>{active ? "進行中的 SESSION" : "準備開始"}</small>
          <strong>{active ? `繼續 ${activeMachine?.nameZh ?? "Session"}` : "開始一局"}</strong>
          <em>{active ? `${active.trackers[activeMachine?.profile.gameTrackers.find(item=>item.primary)?.key??"dataGame"]??active.actualG} G · 投入 ¥${active.investmentYen.toLocaleString()}` : "拍照辨識，或從熟悉的機種開始"}</em>
        </span>
        <b>›</b>
      </Link>
