...
export default function IdentifyPage(){
  const [file,setFile]=useState<File>();const [preview,setPreview]=useState<string>();const [result,setResult]=useState<MachineIdentificationResult>();const [selected,setSelected]=useState(0);const [loading,setLoading]=useState(false);const [error,setError]=useState("");const controller=useRef<AbortController>(null);
  useEffect(()=>()=>{if(preview)URL.revokeObjectURL(preview);controller.current?.abort()},[preview]);
  function pickImage(event:ChangeEvent<HTMLInputElement>){const next=event.target.files?.[0];event.target.value="";if(!next)return;if(!next.type.startsWith("image/")){setError("只能使用圖片檔案。");return}if(preview)URL.revokeObjectURL(preview);setFile(next);setPreview(URL.createObjectURL(next));setResult(undefined);setSelected(0);setError("")}
  function reset(){controller.current?.abort();if(preview)URL.revokeObjectURL(preview);setFile(undefined);setPreview(undefined);setResult(undefined);setSelected(0);setLoading(false);setError("")}
  ...
  return <><PageHeader title="拍機台" eyebrow="AI Machine Identification"/><main className="page pt-5!">
    ...
    <div className={`photo-picker ${preview?"has-photo":""}`}>{preview?<div className="photo-preview" style={{backgroundImage:`url(${preview})`}}/>:<><span>📷</span><strong>拍攝或選擇機台照片</strong><small>建議包含整台、機種名稱與筐體上方</small></>}</div>
    <div className="photo-source-grid"><label>📷 直接拍照<input type="file" accept="image/*" capture="environment" onChange={pickImage}/></label><label>▣ 選擇舊照片<input type="file" accept="image/*" onChange={pickImage}/></label></div>
