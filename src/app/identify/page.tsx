...
export default function IdentifyPage(){
  const kind:"slot"|"pachinko"=usePathname().includes("/pachinko")?"pachinko":"slot";
  ...
  async function submit(){if(!file)return;controller.current=new AbortController();setLoading(true);setError("");try{const next=await identifyMachine(file,controller.current.signal,kind);setResult(next);setSelected(0)}catch(reason){if(reason instanceof DOMException&&reason.name==="AbortError")setError("已取消辨識。");else setError(reason instanceof IdentificationRequestError?reason.message:"辨識服務發生錯誤，請稍後再試。")}finally{setLoading(false)}}
  function cancel(){controller.current?.abort()}
  const candidate=result?.candidates[selected];
  const pachinko=kind==="pachinko";
  return <><PageHeader title={pachinko?"拍照找柏青哥":"拍照找 SLOT"} eyebrow="AI Machine Identification" backHref={pachinko?"/pachinko":"/catalog"} backLabel={pachinko?"返回柏青哥資料庫":"返回 SLOT 資料庫"}/><main className="page pt-5!">
    <div className="notice mb-3">目前只會比對<strong>{pachinko?"柏青哥":"SLOT"}</strong>資料庫，避免同一 IP 的兩種機台互相混淆。 <Link href={pachinko?"/identify":"/identify/pachinko"}>切換至{pachinko?" SLOT":"柏青哥"}辨識</Link></div>
    ...
    {result&&<IdentificationResult kind={kind} result={result} candidate={candidate} selected={selected} onSelect={setSelected} onReset={reset}/>}
  </main></>;
}

function IdentificationResult({kind,result,candidate,selected,onSelect,onReset}:{kind:"slot"|"pachinko";result:MachineIdentificationResult;candidate?:MachineIdentificationCandidate;selected:number;onSelect:(index:number)=>void;onReset:()=>void}){
  const reliable=result.status==="identified"&&candidate;
  const followUp=kind==="slot"?getIdentificationFollowUp(result.status,candidate,new Date().toISOString()):null,wrongKind=result.detectedKind&&result.detectedKind!==kind;
  return <section className="section result-card ai-identification-result">
    ...
    {!reliable&&<div className="uncertain-message"><strong>{wrongKind?`這張照片比較像${result.detectedKind==="pachinko"?"柏青哥":"SLOT"}。`:result.status==="uncertain"?"目前只能確認系列 / IP，無法可靠確認正式機種。":"目前無法辨識機種"}</strong><p>{wrongKind?"已停止目前資料庫的比對，不會因同 IP 自動錯接機台。":"請再拍整台、正式機種名稱、筐體上方或側面銘板，避免只拍角色、演出文字或資料機。"}</p>{wrongKind&&<Link className="secondary-button mt-3" href={result.detectedKind==="pachinko"?"/identify/pachinko":"/identify"}>切換正確辨識</Link>}</div>}
    ...
    {kind==="pachinko"&&reliable&&candidate.matchedCatalogId&&<Link href={`/pachinko/${encodeURIComponent(candidate.matchedCatalogId)}`} className="primary-button mt-4">查看柏青哥機種詳細</Link>}
    ...
    <Link href={kind==="pachinko"?"/pachinko":"/catalog"} className="secondary-button mt-2">⌕ 手動選擇</Link><button className="secondary-button mt-2" onClick={onReset}>📷 重拍／重選</button>
  </section>;
}
