...
export function MachineGuideView({record}:{record:MachineCatalogRecord}){
  ...
  return <main className="page guide-page">
    ...
    <section id="guide-source" className="section card guide-source"><h2>來源與快取</h2><dl><dt>來源</dt><dd><a href={guide.sourceUrl} target="_blank" rel="noreferrer">P-WORLD ↗</a></dd><dt>擷取時間</dt><dd>{new Date(guide.retrievedAt).toLocaleString("zh-TW")}</dd><dt>保存位置</dt><dd>此瀏覽器 localStorage（不跨裝置同步）</dd></dl><button className="secondary-button mt-3" disabled={busy} onClick={refresh}>{busy?"正在重新整理…":"重新整理 P-WORLD 機台指南"}</button>{error&&<div className="guide-error mt-3" role="alert"><strong>重新整理失敗</strong><p>{error}</p><p>目前這份有效指南及所有既有 Session 均已保留。</p></div>}<GuideFeedback catalogId={record.id}/></section>
