...
export function MachineGuideView({record}:{record:MachineCatalogRecord}){
  ...
  return <main className="page guide-page">
    ...
    <div id="guide-details">{player?.sections.map(section=><section className="section card guide-section" key={section.key}><header><div><h2>{section.title}</h2><small>中文重點</small></div></header><p className="guide-summary">{section.summary||"目前尚無可靠說明。"}</p>{section.points.length>0&&<ul>{section.points.map((point,index)=><li key={index}>{point}</li>)}</ul>}</section>)}</div>
