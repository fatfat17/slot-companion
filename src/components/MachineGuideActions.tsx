...
export function MachineGuideActions({record}:{record:MachineCatalogRecord}){
  ...
  return <section className="guide-actions section">
    ...
    </>:pworld?<button className="primary-button" disabled={busy} onClick={()=>rebuild(true)}>{busy?"正在整理公開資料…":cacheState==="stale"?"重新建立機台指南":"建立機台指南"}</button>:<div className="notice">此 Catalog record 沒有可使用的 P-WORLD 機台詳細頁來源。</div>}
    ...
    <p className="guide-cache-note">P-WORLD 為機種主來源；部分試行機種會加入補充攻略來源。單一來源也可使用，本版指南快取不跨裝置同步。</p>
  </section>
}
