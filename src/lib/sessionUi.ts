...
export type SessionUiModel={legacy:boolean;capabilities:SessionCapability[];actionCapabilities:SessionCapability[];counterDefinitions:CounterDefinition[];gameStates:SessionStateControl[];showGenericCz:boolean;showGenericAt:boolean};
...
export function buildSessionUiModel(machine:Machine):SessionUiModel{const snapshot=machine.profile.sessionCapabilities;if(!snapshot)return{legacy:true,capabilities:[],actionCapabilities:[],counterDefinitions:machine.profile.smartCounters.filter(usableCounter),gameStates:LEGACY_STATES,showGenericCz:true,showGenericAt:true};
  ...
  return{legacy:false,capabilities,actionCapabilities,counterDefinitions,gameStates,showGenericCz:actionCapabilities.some(capability=>capability.observationKey==="cz"),showGenericAt:actionCapabilities.some(capability=>capability.observationKey==="at")};
}
