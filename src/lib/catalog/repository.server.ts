...
export interface MachineCatalogRepository{list():Promise<MachineCatalogRecord[]>;search(terms:string[],manufacturers?:string[],limit?:number):Promise<MachineCatalogRecord[]>;approve(decisions:CatalogImportDecision[]):Promise<{processed:number;imported:number;merged:number;skipped:number;total:number}>;recordImportJob(job:CatalogImportJob):Promise<void>}
...
export class SupabaseMachineCatalogRepository implements MachineCatalogRepository{
  ...
  private headers(extra:HeadersInit={}):HeadersInit{return supabaseServerHeaders(this.serviceRoleKey,extra)}
