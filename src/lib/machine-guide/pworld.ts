...
function parseTable(html:string,title:string,index:number,sourceUrl:string):MachineGuideTable|null{const rawRows=[...html.matchAll(/<tr\b[^>]*>([\s\S]*?)<\/tr\s*>/gi)].map(match=>normalizedRow([...match[1].matchAll(/<(th|td)\b[^>]*>([\s\S]*?)<\/\1\s*>/gi)].map(cell=>text(cell[2])))).filter(row=>row.some(Boolean)),headerIndex=rawRows.findIndex(row=>row.length>1&&new Set(row.filter(Boolean)).size>1),headers=rawRows[headerIndex]??[];if(headers.length<2||new Set(headers.filter(Boolean)).size<2)return null;const headerSignature=headers.join("\u0001"),seen=new Set<string>(),rows=rawRows.slice(headerIndex+1).filter(row=>{if(row.length<2||row.join("\u0001")===headerSignature)return false;const populated=row.filter(Boolean),unique=new Set(populated);if(!populated.length||unique.size===1&&populated[0]===title)return false;const signature=row.join("\u0001");if(seen.has(signature))return false;seen.add(signature);return true});if(!rows.length)return null;return{id:`pworld-table-${index}`,title,headers,rows,note:null,sourceUrl}}
