...
const PREFIX="slot-companion-machine-guide-v1:";
export function guideCacheKey(catalogId:string){return`${PREFIX}${catalogId}`}
export function loadCachedGuide(catalogId:string):CachedMachineGuide|null{if(typeof window==="undefined")return null;try{const raw=window.localStorage.getItem(guideCacheKey(catalogId));if(!raw)return null;const parsed=JSON.parse(raw) as CachedMachineGuide;if(parsed.guide?.schemaVersion!==1||parsed.guide.catalogId!==catalogId)return null;return parsed}catch{return null}}
