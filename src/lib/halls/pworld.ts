import type { HallMachineKind,HallMachineResult,HallSearchResult } from "@/types/hall";
...
export function parsePWorldHallList(html:string,sourceUrl:string):HallSearchResult[]{
  ...
  for(const match of html.matchAll(itemPattern)){
    ...
    const pachinkoRates=[...block.matchAll(/<[^>]+data-type=["']p["'][^>]*>([\s\S]*?)<\//gi)].map(item=>decode(item[1])).filter(Boolean);
    const updated=decode(block.match(/<p[^>]+class=["'][^"']*hallList-item-header-update[^"']*["'][^>]*>([\s\S]*?)<\/p>/i)?.[1]??"")||null;
    results.push({name:decode(link.name),address,pworldUrl:absolute(link.href,sourceUrl),nearbyUrl:nearby?absolute(nearby,sourceUrl):null,slotRates:[...new Set(slotRates)],pachinkoRates:[...new Set(pachinkoRates)],updatedLabel:updated});
  }
...
export function parsePWorldHallMachines(html:string,sourceUrl:string,kind:HallMachineKind="slot"):HallMachineResult[]{const results:HallMachineResult[]=[],machineType=kind==="pachinko"?"P":"S",pattern=new RegExp(`<li[^>]+class=["'][^"']*js-hallKisyuList-item[^"']*["'][^>]+data-machine-type=["']${machineType}["'][^>]*>([\\s\\S]*?)<\\/li>`,"gi");for(const match of html.matchAll(pattern)){const opening=match[0].slice(0,match[0].indexOf(">")+1),body=match[1],id=opening.match(/data-machine-id=["'](\d+)["']/i)?.[1]??null,link=body.match(/<p[^>]+class=["'][^"']*_pw-machine-item-machineName[^"']*["'][^>]*>[\s\S]*?<a[^>]+href=["']([^"']+)["'][^>]*>([\s\S]*?)<\/a>/i),name=decode(link?.[2]??"");if(!name)continue;results.push({name,pworldMachineId:id,pworldUrl:absolute(link?.[1]??(id?`/machine/database/${id}`:""),sourceUrl),catalogId:null,catalogName:null})}return results.filter((item,index,array)=>array.findIndex(other=>(item.pworldMachineId&&other.pworldMachineId===item.pworldMachineId)||other.name===item.name)===index)}
